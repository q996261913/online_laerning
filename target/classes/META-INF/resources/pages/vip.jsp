		...
		<div class="j-course-list">
			<div class="uc-course-list g-flow">
				<div class="uc-course-list_content"
					...
					<c:if test="${loginUser!=null}">
						...
						<c:if test="${loginUser.vip!=null }">
							<p style="font-size: 40px;">
								...
								<%
								User loginUser = (User) session.getAttribute("loginUser");//user对象
										out.print(DateFormat.getDateInstance().format(
												loginUser.getVip()));//获取数据库的vip字段并输出
							%>
					...
					<!--半年卡,传递参数1,buyvip(1)-->
					<div style="border: 2px solid; border-radius: 5px; border-color: #EDEDED; box-shadow: 0 0 8px #D7D7D7; margin-top: 80px; position: relative; background-color: white; left: 50px; top: 20px; height: 400px; width: 300px; margin-right: 100px; position: relative; float: left; padding: 10px 20px 40px;">
						<div
							style="text-align: center; padding: 10px 0 20px; border-bottom: 1px solid #ddd; font-size: 20px;">
							半年卡</div>
						<div
